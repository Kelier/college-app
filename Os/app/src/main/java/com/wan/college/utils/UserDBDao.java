...
public class UserDBDao {
    ...
    private static final String TABLE_NAME = "userinfo";//数据表名称
    ...
    //插入一条数据
    public long insertData(User tree) {
    ...
    //更新一条数据
    public long updateData(long id, User tree) {
        ContentValues values = new ContentValues();
     //   values.put(KEY_NAME, tree.getName());
     //   values.put(KEY_AGE, tree.getAge());
     //   values.put(KEY_PRICE, tree.getPrice());
    ...
    //查询一条数据
    public List<User> queryData(long id) {
    ...
    private List<User> convertToTree(Cursor cursor) {
        int resultCounts = cursor.getCount();
        if (resultCounts == 0 || !cursor.moveToFirst()) {
            return null;
        }
        List<User> mTreeList = new ArrayList<>();
