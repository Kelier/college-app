...
public class ParentAdapter extends BaseExpandableListAdapter {
	...
	public View getChildView(final int groupPosition, final int childPosition,
		...
		eListView.setOnGroupExpandListener(new OnGroupExpandListener() {
			...
			public void onGroupExpand(int groupPosition) {
				...
				if (mTreeViewClickListener != null) {
					mTreeViewClickListener.onClickGroupPosition(groupPosition,childPosition,eListView);
				}
			}
		});
	...
	//	mExpandableListView.setGroupIndicator(null);// 取消展开折叠的指示图标
		mExpandableListView.setPadding(mContext.getResources().getDimensionPixelSize(R.dimen.parent_expandable_list_group_padding_left),0,0,0);
		mExpandableListView.setGroupIndicator(mContext.getResources().getDrawable(R.drawable.major_expandlistview_selector));
	...
	public View getGroupView(int groupPosition, boolean isExpanded,
		...
		if (convertView == null) {
			convertView = LayoutInflater.from(mContext).inflate(
					R.layout.item_elist_parent_group, null);
	...
	public interface OnChildTreeViewClickListener {
		...
		void onClickPosition(int parentPosition, int groupPosition,
							 int childPosition);
        void onClickGroupPosition(int parentPosition, int groupPosition, ExpandableListView expandableListView);
	}
